...
class FixedPathParser(ParserBase):
    ...
    def parse_sheet(self, sheet):
        result, is_valid = self.validate_sheet_and_get_data_frame()        
    ...
    def validate_sheet_and_get_data_frame(self):
        ...
        encoding = None
        if file_type is not None and hasattr(file_type, "encoding") and \
           not validator.is_nan(file_type.encoding):
        ...
        mandatory_columns=[]     
        validator.get_mandatory_columns(self.get_sheets()[0]["fields"], mandatory_columns, self.request_context)
    ...
    def get_value(self, from_object, field_source):
        field_index = int(field_source["index"])
        value_object: str = from_object[0]
        
        if len(value_object) < field_index:
            return None 
        
        length: int = int(field_source["length"])
        return value_object[field_index:field_index+length]
        ...
        if "," in encoding:
            ...
            for enc in encodings:
                try:
                    ...
                    log_utils.print_debug_logs("unable to read file from encoding:{}".format(enc), self.request_context)
                    continue
