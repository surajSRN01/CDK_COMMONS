...
def lambda_handler(event, context):
    ...
    bucketParam = ssm.get_parameter(Name='/'+env+'/myapp/scriptbucket', WithDecryption=True)
    target_name = bucketParam['Parameter']['Value']

    # upload glue script to the s3 bucket
    with open("./glue_script.py", "rb") as file:
        s3.upload_fileobj(file, target_name, env +'/scripts/{}'.format("glue_script.py"))
    ...
    runId = glue.start_job_run(JobName=glueJobName, Arguments={'--bucket': bucket_name,
                                                               '--event_key': event_key,
                                                               '--database': database})
    print(runId)
...
def read_event_config_file(key):
    print("inside read event config")
    with open(CONFIG_RULE_LOCATION, "r") as content:
        ...
        for item in array:
            if bool(re.search(item[PATTERN], key)) == True:
                ...
                print("config_file read from s3-event-config-rule.json file")
                return config_file
            

def extract_db(config_file):
    print("inside extract db")
    ...
    for entry in configurations:
        ...
        print("database fetched successfully : "+database)
