    ...
    with open(CONFIG_RULE_LOCATION, "r") as content:
        ...
        for item in array:
            if bool(re.compile(item[PATTERN]).match(key)) == True:
                ...
                upload_file(target_name, job_name, env)
...
def upload_file(target_name, job_name, env):
    # upload glue script to the s3 bucket
    with open("./"+job_name+"/main.py", "rb") as file:
        s3.upload_fileobj(file, target_name, env +'/scripts/{}'.format("glue_script.py"))
            
    # upload zip sample code to s3 bucket
    with open("./"+job_name+".zip", "rb") as zip:
        s3.upload_fileobj(zip, target_name, env +'/package/{}'.format(job_name+".zip"))
